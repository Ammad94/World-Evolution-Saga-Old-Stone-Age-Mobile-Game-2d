    ...
    void Update()
    {
        ...
        if (Mathf.Abs(h) > 0.01f || Mathf.Abs(v) > 0.01f)
        {
            ...
            if (stickX != 0f && stickY != 0f)
            {
                ...
                //
                // IMPORTANT: f + r has magnitude sqrt(2) for pure
                // diagonals (both axes at ±1), which would make the
                // character walk ~41% faster on diagonals than on
                // cardinal keys. Normalize so W, S, A, D, AW, WD,
                // AS, SD all move at the same world-space speed.
                //
                // The SPRITE INDEX is computed in LateUpdate from
                // (Facing vs. camera yaw). The scene sets
                // mirrorLeftRight = 1, which mirrors `rel` for all
                // directions so the screen-relative side is shown
                // (AW -> back_left, WD -> back_right, AS -> front_left,
                // SD -> front_right, A -> left, D -> right).
                Vector3 sum = f * stickY + r * stickX;
                if (sum.sqrMagnitude > 1e-6f)
                {
                    moveDir = sum.normalized;
                    Facing = moveDir;
                }
                else
                {
                    // Degenerate input (camera basis collapsed);
                    // skip this frame to avoid NaN.
                    moveDir = Vector3.zero;
                }
            }
