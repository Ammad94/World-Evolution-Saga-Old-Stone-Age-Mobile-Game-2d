...
public class ThirdPersonCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        // Camera is FIXED — it does NOT auto-follow the character's Facing.
        // Earlier code rotated the camera by 180° when the player pressed S
        // (because Facing flipped to face the camera), which caused the
        // camera to swing around IN FRONT of the character and the
        // character to fill the screen. The user wants the camera to
        // stay behind the character at all times — the camera frame is
        // a deliberate choice (over-the-shoulder GTA view from behind),
        // not something the character's Facing should drag around.
        //
        // If you ever want A/D to swing the camera, enable the auto-
        // follow and gate it to "only when |delta| > 90° AND the
        // character is NOT walking toward/away from the camera" — but
        // that is not a current requirement.
        if (false && autoFollowFacing && !oi.active && isMoving) // disabled
        {
    ...
    Vector3 GetTargetFacing()
    {
        ...
        {
            Vector3 v = rb.linearVelocity; v.y = 0; return v.normalized;
        }
        var rb2d = target.GetComponent<Rigidbody2D>();
        if (rb2d != null && rb2d.linearVelocity.sqrMagnitude > 0.01f)
        {
            Vector3 v = rb2d.linearVelocity; return v.normalized;
        }
    ...
    bool IsTargetMoving()
    {
        ...
        var pc3d = target.GetComponent<PlayerController3D>();
        if (pc3d != null) return pc3d.Facing.sqrMagnitude > 0.01f && (target.GetComponent<Rigidbody>()?.linearVelocity.sqrMagnitude > 0.01f || true);
        // check via position delta or input
        var rb = target.GetComponent<Rigidbody>();
        if (rb != null) return rb.linearVelocity.magnitude > 0.1f;
        var rb2d = target.GetComponent<Rigidbody2D>();
        if (rb2d != null) return rb2d.linearVelocity.magnitude > 0.1f;
